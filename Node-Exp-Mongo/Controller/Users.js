...
exports.userLogin = async (req, res) => {
    try {
        ...
        const emailFound = await userRegisterationModel.findOne({ email: req.body.email });
        if (!emailFound) {
            return res.status(200).json({ message: "Email does not exists. Please enter correct Email" });
        }

        const userNameFound = await userRegisterationModel.findOne({ username: req.body.username });
        if (!userNameFound)
            return res.status(200).send({ message: "userName does not exists. Please enter correct Username" });
        const passwordvalidation = await bcrypt.compare(req.body.password, userNameFound.password);
        if (!passwordvalidation)
            return res.status(200).json({ message: "Password in correct" });

        const token = jwt.sign({ userNameFound: userNameFound.username }, process.env.CLIENT_KEY);
        return res.header('access-token', token).send({ jwt: token, message: "You have been Logged in Successfully" });
